...
class PrivateRecipeAPITest(TestCase):
    ...
    def update_user_returns_error(self):
        """
        Test changing the user for a recipe results in error
        """
        new_user = create_user(
            email='user2@example.com', password='testpass2123'
        )
        recipe = create_recipe(user=self.user)
    ...
    def test_delete_recipe(self):
        """
        Test ability to delete a recipe
        """
        recipe = create_recipe(user=self.user)
        url = detail_url(recipe.id)
    ...
    def test_delete_other_users_recipe_error(self):
        """
        Test attempt to delete another users recipe results in error
        """
        new_user = create_user(
            email='user2@example.com', password='testpass2123'
        )
